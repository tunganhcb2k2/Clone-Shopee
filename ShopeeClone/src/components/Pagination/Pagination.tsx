...
export default function Pagination({ queryConfig, pageSize }: Props) {
  ...
  return (
    <div className='flex flex-wrap mt-6 justify-center'>
      {page === 1 ? (
        <span className='bg-white rounded px-3 py-2 shadow-sm mx-2 cursor-not-allowed border bg-white/60'>Prev</span>
      ) : (
        <Link
          to={{
            pathname: path.home,
      ...
      {renderPagination()}
      {page === pageSize ? (
        <span className='bg-white rounded px-3 py-2 shadow-sm mx-2 cursor-not-allowed border bg-white/60'>Next</span>
      ) : (
        <Link
          to={{
            pathname: path.home,
